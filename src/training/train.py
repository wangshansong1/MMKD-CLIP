...
def save_checkpoint_kd(model, optimizer, scaler, epoch, i, args):
    checkpoint_dict = {
        "epoch": epoch,
        "epoch_step": i,  # inner loop saves step and args.resume in main.py will decide if a checkpoint is saved by innerloop or epoch loop (in main).
        "name": args.name,
    ...
    if args.save_logs:  # master_only.
        ...
        if args.save_most_recent:
            torch.save(
                checkpoint_dict,
                os.path.join(args.checkpoint_path, f"epoch_kd_latest.pt"),
            )
...
def to_device_kd(batch, device, args):

    images, texts, if_512, tf_512 = batch
    images = images.to(device=device, non_blocking=True)
    if hasattr(args, "inmem") and args.inmem:
        images = images.to(torch.float32).div_(255.)  # b, 3, 224, 224
        mean, std = get_mean_std(args)
        ...
        images.sub_(mean).div_(std)
    texts = texts.to(device=device, non_blocking=True)
    if_512 = if_512.to(device=device, dtype=torch.bfloat16, non_blocking=True)

    tf_512 = tf_512.to(device=device, dtype=torch.bfloat16, non_blocking=True)
    return images, texts, if_512, tf_512




def train_one_epoch_ex(model, _, data, epoch, epoch_step, optimizer, scaler, scheduler, args, tb_writer=None):
...
def train_one_epoch_kd(model, losskd, data, epoch, epoch_step, optimizer, scaler, scheduler, args, tb_writer=None):
    device = torch.device(args.device)
    autocast = torch.cuda.amp.autocast if args.precision == 'amp' else suppress
    ...
    for i in range(num_batches_per_epoch):
        ...
        img, txt, imgfea, txtfea = to_device_kd(batch, device, args)

        data_time_m.update(time.time() - end)
        optimizer.zero_grad()

        with autocast(dtype=torch.bfloat16):
            image_features_512, text_features_512, logit_scale_512 = model(img, txt)

            task_loss, fd_loss, icl_loss = losskd(image_features_512, text_features_512, logit_scale_512,
                                  imgfea, txtfea)

            
            total_loss = task_loss + args.kdweight * (fd_loss) + icl_loss
        ...
        if is_master(args) and (i % 100 == 0 or batch_count == num_batches_per_epoch):
            ...
            # NOTE loss is coarsely sampled, just master node and per log update
            loss_m.update(total_loss.item(), batch_size)
            fdloss.update((args.kdweight * fd_loss).item(), batch_size)
            # fdloss.update((0 * fd_loss).item(), batch_size)
            iclloss.update((icl_loss).item(), batch_size)
            logging.info(
                f"Train Epoch: {epoch} [{num_samples:>{sample_digits}}/{samples_per_epoch} ({percent_complete:.0f}%)] "
                f"Loss: {loss_m.val:#.5g} ({loss_m.avg:#.4g}) "
                f"FD Loss: {fdloss.val:#.5g} ({fdloss.avg:#.4g}) "
                f"ICL Loss: {iclloss.val:#.5g} ({iclloss.avg:#.4g}) "
                f"Data (t): {data_time_m.avg:.3f} "
                f"Batch (t): {batch_time_m.avg:.3f}, {args.batch_size*args.world_size / batch_time_m.val:#g}/s "
                f"LR: {optimizer.param_groups[0]['lr']:5f} "
            )
        ...
        if hasattr(args, "save_steps") and (step + 1) % args.save_steps == 0:
            save_checkpoint_kd(model, optimizer, scaler, epoch, i, args)
