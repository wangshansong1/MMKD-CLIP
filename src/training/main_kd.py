...
def save_checkpoint(model, optimizer, scaler, completed_epoch, args):
    ...
    if args.save_logs:
        if completed_epoch == args.epochs or (
            ...
            torch.save(
                checkpoint_dict,
                os.path.join(args.checkpoint_path, f"epoch_kd_{completed_epoch}.pt"),
            )
...
def main(args=None):
    ...
    if args.distributed and not args.horovod:
        ...
        if args.distributed_engine == 'ddp':
            ...
            model = torch.nn.parallel.DistributedDataParallel(model, broadcast_buffers=False, device_ids=[device], **ddp_args)
        else:
    ...
    if hasattr(args, "loss"):
        ...
        loss_cls = getattr(loss, "KDClipLoss")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if hasattr(args, "engine"):
            ...
            engine_cls(model, losskd, data, epoch, epoch_step, optimizer, scaler, scheduler, args, writer)
        else:
...
if __name__ == "__main__":
    ...
    config.logs = '/path/result/' + exp_name
    config.output_dir = os.path.join(config.logs, config.name)
    config.pretrained = os.path.join(sys.argv[3])
    config.data_root = '/path/MMKDCLIP_DATA'
    config.teacherchoose = 'meta'
